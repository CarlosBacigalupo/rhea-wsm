...
def do_read_calibration_file(arcFile, modelXMLFile, outputFileName, sexParamFile, finalOutputFileName, SEDMap, booAnalyse=True, booAvgAdjust = True, booPlotInitialPoints = False, booPlotFinalPoints = False):
    ...
    imageMapX, imageMapY, image_map_sigx, image_map_sigy = wt.ia.read_image_map_sex(outputFileName)
    ...
    imageMapWavelength = wt.ia.identify_imageMapWavelength_avg(SEDMap, CCDX, CCDY, CCDLambda, imageMapX, imageMapY, booAvgAdjust = booAvgAdjust)
    
    #Create temporary output file with all calibration data
    f = open(finalOutputFileName,'w')
    for i in range(len(imageMapX)):
        out_string = str(imageMapX[i]) + ' ' + str(imageMapY[i]) + ' ' + str(imageMapWavelength[i]) + ' ' + str(image_map_sigx[i]) + ' ' + str(image_map_sigy[i]) + '\n'
        f.write(out_string) 
    f.close()

    #Correct/confirm found wavelengths
#    imageMapWavelength = wt.ia.identify_imageMapWavelength_manual(SEDMap, CCDX, CCDY, CCDLambda, imageMapX, imageMapY, imageMapWavelength, arcFile, Cameras)
...
def do_plot_sed_map(SEDMap, title='', point_density=1):
        ...
        colorTable = np.array((wt.optics.wav2RGB(SEDMap[SEDMapLambda], SEDMap[SEDMapIntensity]))) 
...
#        plt.ion()
        ...
        if booLabels:
            fullLabel = [str(imageMapWavelength[x]) for x in np.arange(len(imageMapWavelength))]
