...
for i in range(12):
    JD = gd2jd((2013, i, 24, 21, 25, 00),TZ=11)
    
    vh, vb = baryvel(JD)
    # print vh, vb
    raDeg = (0*15+24/60+5.67/3600)
    decDeg = (-72+4/60+52.6/3600)
    ra = deg2rad(raDeg)    #RA  in radians
    dec = deg2rad(decDeg)  #Dec in radians
    
    v = vb[0]*cos(dec)*cos(ra) + vb[1]*cos(dec)*sin(ra) + vb[2]*sin(dec)
    
#     print (vh[0]**2+vh[1]**2+vh[2]**2)**(0.5)
#     print (vb[0]**2+vb[1]**2+vb[2]**2)**(0.5)
    print vb
# print 'Earth rv towards 47 tuc (km/s):',v